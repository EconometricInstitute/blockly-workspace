<template>
    <v-container>
        <v-row v-if="type == 'text' || type == 'number' || type =='boolean'">
            ...
            <v-select density="compact" v-if="type == 'boolean'" label="Value" :items="[{title: 'false', value: false}, {title: 'true', value: true}]" :model-value="value" @update:modelValue="updateValue($event)"/>
        </v-row>
        <template v-else>
